...
return {
  {
    ...
    init = function()
      local recommendations = {
        ...
        zsh = { "shellcheck" }, -- no shfmt: it has no zsh dialect (conform excludes zsh too)
      ...
      vim.api.nvim_create_user_command("MasonSuggest", function()
        ...
        if not pkgs or #pkgs == 0 then
          vim.notify(
            ("No mason suggestions for filetype %q"):format(ft),
            vim.log.levels.WARN,
            { title = "MasonSuggest" }
          )
          return
        ...
        if not ok then
          vim.notify("mason-registry not available", vim.log.levels.ERROR, { title = "MasonSuggest" })
          return
        end
        ...
        if #items == 0 then
          vim.notify("No matching mason packages found", vim.log.levels.WARN, { title = "MasonSuggest" })
          return
        end
